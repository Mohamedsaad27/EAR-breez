            ...
            <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        <tr>
                            <th scope="col" class="px-4 py-3">ProductId</th>
                        ...
                        <tr class="border-b dark:border-gray-700">
                            ...
                            <td class="px-4 py-3 ">{{$product->quantity}} PCS</td>
                            <td class="px-4 py-3">
                                @switch(strtolower($product->status))
                                    @case('in stock')
                                        <span class="text-green-500 font-semibold">{{ ucfirst($product->status) }}</span>
                                        @break
                                    @case('out stock')
                                        <span class="text-red-500 font-semibold">{{ ucfirst($product->status) }}</span>
                                        @break
                                    @case('limited stock')
                                        <span class="text-orange-500 font-semibold">{{ ucfirst($product->status) }}</span>
                                        @break
                                    @default
